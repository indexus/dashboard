...
/** All-nodes rollup: sum counters, average percentages. */
...
export default function OpsPanel({ mesh, onSelectCollection }) {
  ...
  const [keepSnapshots, setKeepSnapshots] = useState(true);
  ...
  // Live only while a node row is selected; otherwise Network / Data / Snapshots.
  useEffect(() => {
    if (selected) {
      setSideTab("live");
      return;
    }
    setSideTab((t) => (t === "live" ? "config" : t));
  }, [selected]);
  ...
  useEffect(() => {
    ...
    drawChart(
      memRef.current,
      hist.map((h) => h.mem_pct),
      "#5eb8e8",
      { max: 100, limit: lim.mem_limit_pct },
    );
    drawChart(
      cpuRef.current,
      hist.map((h) => h.cpu_pct),
      "#2ec4a8",
      { max: 100, limit: lim.cpu_limit_pct },
    );
    const iSeries = hist.map((h) => h.items);
    const peak = Math.max(0, ...iSeries.filter((v) => v != null));
    const iMax = Math.max(peak * 1.1, 100);
    drawChart(itemsRef.current, iSeries, "#e89b3c", {
      max: iMax,
      limit: lim.items_limit ?? null,
    });
  }, [mesh, stats, statsScope, lim.mem_limit_pct, lim.cpu_limit_pct, lim.items_limit]);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        setDvfStatus(s);
        if (s.running) {
          setDvfMsg(progressLabel(s.progress));
        } else if (s.error) {
  ...
  async function onRemesh({ keep_snapshots = true } = {}) {
    ...
    if (
      !confirm(
        "Restart the local mesh?\n\nThis stops issuer + bootstrap + spawned nodes, then runs mesh_up with your Config. Node data under .data-local/nodes is wiped (snapshots kept only if checked).",
      )
    ) {
      return;
    }
    setRemeshBusy(true);
    setRemeshMsg("writing config · restarting mesh…");
    try {
        ...
        <div className="charts charts--3">
          <div className="chart-panel">
            <div className="chart-label">
              <span>memory{selected ? "" : " · avg"}</span>
              <span className="now">
                {stats.mem_pct != null ? `${fmt(stats.mem_pct)}%` : "—"}
              </span>
            </div>
            <canvas className="chart" ref={memRef} width={320} height={88} />
          </div>
          <div className="chart-panel">
            <div className="chart-label">
              <span>cpu{selected ? "" : " · avg"}</span>
              <span className="now">
                {stats.cpu_pct != null ? `${fmt(stats.cpu_pct)}%` : "—"}
              </span>
            </div>
            <canvas className="chart" ref={cpuRef} width={320} height={88} />
          </div>
          <div className="chart-panel">
            <div className="chart-label">
              <span>items{selected ? "" : " · sum"}</span>
              <span className="now">
                {stats.items != null ? fmt(stats.items) : "—"}
              </span>
            </div>
            <canvas className="chart" ref={itemsRef} width={320} height={88} />
          </div>
        </div>
      </div>
      ...
      <div className="ops-nodes-layout">
        <div className="ops-nodes-col">
          <div className="section-head">
            <h2>nodes</h2>
            <span className="hint">{spawnStatus}</span>
          </div>
